...
export default async function GuidePage({ params }: Props) {
  ...
  //
  // Per-guide image priority:
  //  1. Frontmatter `ogImage` if set — author override (absolute URL or path).
  //  2. The per-slug opengraph-image route at the same path. Renders the
  //     guide's title onto a PlanSight-branded 1200x630 PNG via the
  //     ImageResponse API in opengraph-image.tsx adjacent to this file.
  //  3. (No third fallback needed — the route always renders, even if
  //     getGuide() returns null in the OG component.)
  const imageUrl = guide.ogImage
    ? guide.ogImage.startsWith("http")
      ? guide.ogImage
      : `${SITE_URL}${guide.ogImage}`
    : `${SITE_URL}/products/plansight-ai/guides/${slug}/opengraph-image`;
